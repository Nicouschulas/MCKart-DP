...
data modify storage temp list[0].data.icon.components display.Lore prepend value '{"text":" "}'
...
execute unless data storage temp list[0].data.children[0] run item modify block 0 0 0 container.0 minecraft:cosmetics/amount
...
execute if data storage temp list[0].data.children[0] run item replace entity @s inventory.13 with gold_nugget{display:{Name:'{"text":" "}'},menu:1b,killme:1b,CustomModelData:137}
