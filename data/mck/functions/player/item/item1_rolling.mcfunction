item replace block 0 0 0 container.0 with carrot_on_a_stick[minecraft:item_name='{"translate":"race.item.rolling","color":"dark_gray","italic":false}',minecraft:custom_model_data=1001,minecraft:unbreakable={},minecraft:custom_data={killme:1b}] 1
execute store result score custommodeldata mck_temp run data get entity @s EnderItems[3].components."minecraft:custom_data".item1[0] 1
