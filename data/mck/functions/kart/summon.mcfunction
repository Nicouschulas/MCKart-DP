...
execute if score bool mck_temp matches 1 run execute as @e[tag=temp,type=fox,limit=1,sort=nearest] store result entity @s HandItems[0].tag.CustomModelData int 1 run scoreboard players get temp mck_temp  
execute if score bool mck_temp matches 1 run execute as @e[tag=temp,type=fox,limit=1,sort=nearest] run scoreboard players operation @s mck_id = temp mck_temp  
